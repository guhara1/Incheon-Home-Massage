...
# 공통 평점·후기 (샘플) — 모든 페이지 Service 스키마에 부착.
# 주의: 실제 수집 후기 데이터로 교체 전까지는 샘플 값이다.
AGGREGATE_RATING = {"value": "4.8", "count": "137", "best": "5", "worst": "1"}
SAMPLE_REVIEWS = [
    ("김민서", "5", "2026-04-12",
     "예약 전에 방문 가능 지역과 예약 가능 시간을 정확히 안내받아 편리했습니다. 추가 이동비 기준도 미리 알려주셔서 좋았어요."),
    ("이도현", "5", "2026-03-28",
     "건물 출입 방식과 방문 주소 확인 과정이 꼼꼼했습니다. 안내된 관리 범위 안에서 안전하게 이용했습니다."),
    ("박서연", "4", "2026-02-19",
     "상담이 친절했고 예약 변경 기준도 명확했습니다. 생활권별 안내가 잘 정리되어 있어 위치 확인이 쉬웠습니다."),
]
...
def make_service_schema(title: str, canonical: str) -> dict:
    ...
    return {
        ...
        "@type": "Service",
        "name": title,
        "serviceType": "방문 관리 서비스(출장마사지·홈타이)",
        "url": canonical,
        ...
        "review": [
            ...
            for name, score, date, body in SAMPLE_REVIEWS
        ],
    }
...
def render_page(page: dict) -> str:
    ...
    # 스키마 자동 주입 (모든 페이지).
    # 메인(hero 보유)은 main.py의 extra_head에 WebSite/WebPage/ImageObject/FAQ가 이미 있으므로
    # Organization + Service(평점·후기)만 보강하고,
    # 나머지 페이지는 Organization + WebPage + Service(평점·후기) + BreadcrumbList를 생성한다.
    if hero:
        blocks = [make_org_schema(), make_service_schema(title, canonical)]
    else:
        blocks = [
            make_org_schema(),
            make_webpage_schema(title, desc, canonical),
            make_service_schema(title, canonical),
        ]
        if crumbs:
...
<footer class="site-footer">
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <p class="footer-desc">{SERVICE_AREA_TEXT} 방문 출장마사지·홈타이 안내 사이트입니다. 모든 서비스는 안내된 관리 범위와 위생·안전 기준 안에서만 제공됩니다.</p>
      <address class="footer-contact">
        ...
        <span class="footer-contact-row"><span class="footer-label">서비스 지역</span> {SERVICE_AREA_TEXT}</span>
      </address>
    </div>
  ...
  <div class="footer-bottom">
    <div class="container footer-bottom-inner">
      ...
      <div class="footer-actions">
        <a class="btn-telegram" href="{TELEGRAM_URL}" target="_blank" rel="noopener nofollow" title="웹사이트 제작문의 텔레그램">📱 웹사이트 제작문의</a>
        <a class="btn-partnership" href="{TELEGRAM_URL}" target="_blank" rel="noopener nofollow" title="제휴문의 텔레그램">🤝 제휴문의</a>
      </div>
    </div>
  </div>
</footer>
